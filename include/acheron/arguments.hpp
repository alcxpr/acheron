...
namespace ach
{
	...
	public:
		...
		friend std::basic_ostream<CharT, Traits> &operator<<(std::basic_ostream<CharT, Traits> &os, const argument &arg)
		{
	...
	private:
		using storage_type =
						std::vector<argument, typename std::allocator_traits<Allocator>::template rebind_alloc<argument>>;
		using string_storage_type = std::vector<
						typename argument::string_type,
						typename std::allocator_traits<Allocator>::template rebind_alloc<typename argument::string_type>>;
	...
	public:
		...
		explicit arguments(const Allocator &alloc) : args(alloc), strings(alloc)
		{
			d::initialize_args();
