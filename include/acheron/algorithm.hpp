...
namespace ach 
{
    ...
    template<std::input_iterator It, std::sentinel_for<It> S, typename F>
    constexpr std::size_t hash_combine(It first, S last, F&& f)
    {
    ...
    {
        template<std::ranges::input_range R, typename F>
        constexpr std::size_t hash_combine(R&& r, F&& f)
        {
    ...
    template<std::forward_iterator It, std::sentinel_for<It> S, typename Comp = std::ranges::less>
    ...
    namespace ranges 
    {
        template<std::ranges::forward_range R, typename Comp = std::ranges::less>
    ...
    constexpr To safe_cast(const From& src) noexcept
    {
        ...
        /* note: add conditional compilation for runtime error since the error messages are 
         *  nicer than the compile error */
    }

    /* more casting utilities should go here in the future e.g. `reference_cast<T>(From&)` */
}
