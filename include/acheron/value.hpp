...
namespace ach
{
	...
	private:
		...
		static constexpr vtable_t vtable_for = {
			...
			.copy = [](pointer dst, const_pointer src) noexcept(std::is_nothrow_copy_constructible_v<T>)
			{ std::construct_at(static_cast<T *>(dst), *static_cast<const T *>(src)); },
			.move = [](pointer dst, pointer src) noexcept(std::is_nothrow_move_constructible_v<T>)
		...
		template<typename T>
		static constexpr bool fits_in_buffer = sizeof(T) <= buffer_size && alignof(T) <= buffer_align;
		...
		[[nodiscard]] bool uses_buffer() const noexcept
		{
			...
			auto offset = static_cast<size_type>(static_cast<const byte_type *>(data) - buffer);
		...
		pointer allocate_for_type()
		{
			...
			{
				static_assert(fits_in_buffer<T>, "Over-aligned types must fit in buffer or use aligned allocator");
	...
	public:
		...
		value(const value &other) : vtable(other.vtable)
		{
			...
			data = other.uses_buffer() ? static_cast<pointer>(buffer)
		...
		value &operator=(const value &other)
		{
			if (this != &other)
			{
				...
				data = other.uses_buffer() ? static_cast<pointer>(buffer) : allocator_traits::allocate(allocator, vtable->size);
		...
		void emplace(Args &&...args) noexcept(std::is_nothrow_constructible_v<typename value_traits<T>::value, Args...> &&
																					fits_in_buffer<typename value_traits<T>::value>)
		{
			...
			data = fits_in_buffer<wrapper_type> ? static_cast<pointer>(buffer) : allocate_for_type<wrapper_type>();
