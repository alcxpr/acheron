...
namespace ach
{
	...
	public:
		 ...
		 */
		freelist(freelist &&other) noexcept(std::is_nothrow_move_constructible_v<allocator_type> &&
																				std::is_nothrow_move_constructible_v<std::vector<node_type *>>) :
				allocator(std::move(other.allocator)), blocks(std::move(other.blocks)),
				head(std::exchange(other.head, nullptr)), in_use(std::exchange(other.in_use, 0)),
		 ...
		 */
		freelist &operator=(freelist &&other) noexcept(std::is_nothrow_move_assignable_v<allocator_type> &&
		...
		pointer emplace(Args &&...args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
